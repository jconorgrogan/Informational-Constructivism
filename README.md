...
1.  **Concrete Difference is Logically Necessary:** Reality cannot logically be pure nothingness (**S₀** – sterile, unreferable) nor a static "everything-at-once" (**Ω** – paradoxical simultaneity). The incoherence of these extremes forces the emergence of *at least one* specific, finite distinction (**F-0**) – a basic "this" versus "not this." This minimal difference requires reality to operate with a finite set of distinguishable operational states or labels (**Finite Representational States, FRS / S-FiniteQ**).
2.  **Finite Dynamics Force Memory & Time (L1/LR):** Any system capable of change (**S-Carrier** enabling Sequences, SE) and operating with only a finite set of labels (**FRS / S-FiniteQ**) will *mathematically inevitably* be forced to reuse the same label for different underlying events as it evolves (Pigeonhole Principle). This compelled reuse (**Label Recurrence, L1/LR**) – treating difference as sameness due to finite representation – is the fundamental origin of internal sequence tracking, memory-as-compression (structured forgetting), and thus, internal time.
3.  **Irreversibility & Finite Limits Force Structure (SUR):** State transitions within a definite (FRS) system inherently involve **Necessary Irreversibility (NI)** (e.g., information erasure in overwrites isn't trivially reversible). NI leads to unavoidable **Necessary Operational Consequences (NOC)**, such as path dependence and minimum operational steps for cycles. Systems facing increasing complexity eventually hit representational limits (**Saturation, L5/SP**) where FRS is insufficient. To continue operating persistently (**Condition C2/PF**), such systems *must* navigate these limits by reusing FRS states via pathways (e.g., overwriting vs. encoding) that manage the inherent trade-offs dictated by NOC. The resulting dynamic, characterized by trajectories shaped by these necessary trade-offs under FRS and persistence constraints, **is SUR (Structure Under Resource-constraint)**. SUR drives the emergence of internal structure, rules, and complexity as a consequence of navigating fundamental operational limits.
